...
describe("ensureMonthsContinuity", () => {
  ...
  it("should do nothing if months are already up to date", async () => {
    ...
    (categoryRepository.getMostRecentMonths as jest.Mock).mockResolvedValue([
      { categoryId: "cat-1", month: currentMonth, available: new Prisma.Decimal(100) },
    ]);
  ...
  it("should create a single missing month for all categories", async () => {
    ...
    (categoryRepository.getMostRecentMonths as jest.Mock).mockResolvedValue([
      { categoryId: rtaCategoryId, month: lastMonth, available: new Prisma.Decimal(-50) },
      { categoryId: "cat-2", month: lastMonth, available: new Prisma.Decimal(100) },
      { categoryId: "cat-3", month: lastMonth, available: new Prisma.Decimal(-20) }, // Should carry over 0
    ]);
    ...
    (categoryRepository.getRtaCategoryId as jest.Mock).mockResolvedValue(rtaCategoryId);

    await ensureMonthsContinuity(mockPrisma, userId);

    expect(categoryRepository.createMonths).toHaveBeenCalledTimes(1);
    const createdMonths: Prisma.MonthCreateManyInput[] = (categoryRepository.createMonths as jest.Mock).mock.calls[0][1];
  ...
  it("should create multiple missing months", async () => {
    ...
    (categoryRepository.getMostRecentMonths as jest.Mock).mockResolvedValue([
      { categoryId: "cat-1", month: lastMonth, available: new Prisma.Decimal(100) },
    ]);

    (categoryRepository.getAllCategoryIds as jest.Mock).mockResolvedValue([{ id: "cat-1" }]);
    (categoryRepository.getRtaCategoryId as jest.Mock).mockResolvedValue(rtaCategoryId);
