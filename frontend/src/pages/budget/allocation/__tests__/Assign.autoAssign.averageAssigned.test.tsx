...
import {
  averageAssignedData,
  averageAssignedAlreadyMatchedData,
...
const buttonText = "average assigned";

describe("Assign - Average Assigned", () => {
  const server = setupTestServer();

  beforeEach(() => {
    server.use(
      http.get(`${API_URL}/budget/category`, () => {
        return HttpResponse.json(averageAssignedData);
      })
    );
  });
    ...
    describe("previous months present", () => {
      describe("button", () => {
        it("button displays correct amount", async () => {
          ...
          await expectButtonAmount(new RegExp(`${buttonText} £97\.50`, "i"));
        });
      });
      describe("modal", () => {
        it("displays categories to update correctly", async () => {
          ...
          await expectCategoryRow("Groceries", "+£77.50");
        });

        it("displays will not update message when no months to update", async () => {
          server.use(
            http.get(`${API_URL}/budget/category`, () => {
              return HttpResponse.json(averageAssignedAlreadyMatchedData);
            })
          );
