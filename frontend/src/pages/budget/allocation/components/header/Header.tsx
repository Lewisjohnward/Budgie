    ...
    <div className="flex gap-4 items-center">
      <div className="flex items-center gap-1">
        <button
          ...
          className={`rounded-xl w-6 h-6 ${!month.canGoPrev
              ? "opacity-50"
        ...
        <button
          ...
          className={`rounded-xl w-6 h-6 ${!month.canGoNext
              ? "opacity-50"
              : `hover:${darkBlueBgHover} cursor-pointer`
            }`}
        >
      ...
      <button
        ...
        className={`px-2 py-1 rounded ${month.isCurrentMonth
            ? "opacity-0"
...
export function AssignedMoney({ amount }: { amount: number }) {
  ...
  return (
    <div
      className={`hidden md:flex items-center gap-8 ${state.bg} rounded px-4 py-2`}
    >
      <div>
